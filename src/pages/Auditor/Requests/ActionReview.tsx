...
const ActionReview = () => {
  ...
  // Fetch attachments for selected finding only
  ...
  // Calculate attachment status summary
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6">
              ...
              <div className="mb-4 sm:mb-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 gap-2">
                  <h3 className="font-semibold text-gray-900 text-sm sm:text-base">Evidence Submitted</h3>
                  {findingAttachments.length > 0 && (
                    <div className="flex flex-wrap gap-1 sm:gap-2 text-xs">
                      <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded whitespace-nowrap">📂 {attachmentSummary.total}</span>
                      <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded whitespace-nowrap">🔵 {attachmentSummary.open}</span>
                      <span className="px-2 py-1 bg-green-100 text-green-800 rounded whitespace-nowrap">✅ {attachmentSummary.approved}</span>
                      <span className="px-2 py-1 bg-red-100 text-red-800 rounded whitespace-nowrap">🔄 {attachmentSummary.returned}</span>
                    </div>
                  )}
                </div>
                {findingAttachments.length === 0 ? (
            ...
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200 bg-gray-50">
              <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3">
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4">
                  ...
                  {selectedAction.status === 'Active' && attachmentSummary.total > 0 && attachmentSummary.approved < attachmentSummary.total && (
                    <div className="text-xs sm:text-sm text-amber-600 flex items-center gap-2">
                      ⚠️ Warning: Not all attachments are approved ({attachmentSummary.approved}/{attachmentSummary.total})
                    </div>
                  )}
                </div>
