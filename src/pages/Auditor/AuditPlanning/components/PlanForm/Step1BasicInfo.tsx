  ...
  useEffect(() => {
    const checkTimeConflict = async () => {
      ...
      try {
        ...
        // Only consider audits with status 'Approved' or 'InProgress' when checking for conflicts
        ...
        // Rule 1: Chỉ warning khi trùng time nhưng KHÁC department
        // Nếu có selectedDeptIds, check xem có audit nào trùng time nhưng khác department không
        if (level === 'department' && selectedDeptIds.length > 0 && overlappingAudits.length > 0) {
          // Check which audits have different departments
          const auditsWithDifferentDepts: any[] = [];
          ...
          for (const audit of overlappingAudits) {
            try {
              ...
              // Check if audit has any matching department
              const hasMatchingDept = scopeDeptArray.some((sd: any) => 
                selectedDeptIds.includes(String(sd.deptId))
              );
              
              // Only count as conflict if time overlaps but departments are different
              if (!hasMatchingDept) {
                auditsWithDifferentDepts.push(audit);
              }
            } catch (err) {
              // If we can't check, assume different department (safer to warn)
              auditsWithDifferentDepts.push(audit);
            }
          }
          
          if (auditsWithDifferentDepts.length > 0) {
            setHasTimeConflict(true);
            setConflictingAuditsCount(auditsWithDifferentDepts.length);
          } else {
            setHasTimeConflict(false);
            setConflictingAuditsCount(0);
          }
        } else if (overlappingAudits.length > 0) {
          // For academy level or no departments selected, show warning for any time overlap
          setHasTimeConflict(true);
    ...
    checkTimeConflict();
  }, [periodFrom, periodTo, editingAuditId, level, selectedDeptIds]);
