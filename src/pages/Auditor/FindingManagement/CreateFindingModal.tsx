              ...
              <div className="bg-red-50 border-2 border-red-300 rounded-lg p-4 space-y-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="text-sm font-semibold text-red-900 mb-1">Previous Finding Was Rejected by Witness</h3>
                    <div className="bg-white border border-red-200 rounded p-3 mb-2">
                      <p className="text-sm text-red-800">
                        <span className="font-medium">Rejection Reason: </span>
                        {rejectedFinding.witnessDisagreementReason || 'No reason provided'}
                      </p>
                    </div>
                    <details className="mt-2">
                      <summary className="text-sm font-medium text-red-900 cursor-pointer hover:text-red-700">
                        View Previous Finding Details
                      </summary>
                      <div className="mt-2 pl-4 border-l-2 border-red-300 space-y-2">
                        <div>
                          <p className="text-xs font-medium text-gray-700">Finding Description:</p>
                          <p className="text-sm text-gray-600">{rejectedFinding.findingDescription}</p>
                        </div>
                        {rejectedFinding.criteriaNotMet && (
                          <div>
                            <p className="text-xs font-medium text-gray-700">Criteria Not Met:</p>
                            <p className="text-sm text-gray-600">{rejectedFinding.criteriaNotMet}</p>
                          </div>
                        )}
                        {rejectedFinding.severity && (
                          <div>
                            <p className="text-xs font-medium text-gray-700">Severity:</p>
                            <p className="text-sm text-gray-600">{rejectedFinding.severity}</p>
                          </div>
                        )}
                      </div>
                    </details>
                    <p className="text-xs text-red-700 mt-2">
                      ⚠️ Please review the rejection reason and update your finding accordingly before resubmitting.
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  {aiAnalysisSummary && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p className="text-sm text-yellow-800 whitespace-pre-wrap">{aiAnalysisSummary}</p>
                        </div>
                      </div>
                    </div>
                  )}
